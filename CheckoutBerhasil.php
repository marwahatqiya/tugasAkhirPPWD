<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Checkout Berhasil</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
    <style>
        body { background: #f5f5f5; height: 100vh; display: flex; align-items: center; justify-content: center; text-align: center; }
        .card { padding: 50px; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border: none; }
        .icon-success { font-size: 80px; color: #28a745; }
        .btn-home { background: #427AB5; color: white; border-radius: 10px; padding: 10px 30px; text-decoration: none; }
    </style>
</head>
<body>
    <div class="card">
        <div class="icon-success"><i class="bi bi-check-circle-fill"></i></div>
        <h1 class="fw-bold mt-3">Checkout Berhasil!</h1>
        <p class="text-muted">Impianmu selangkah lebih dekat. Tabungan telah dipotong.</p>
        <div class="mt-4">
            <a href="dashboard.php" class="btn-home">Kembali ke Dashboard</a>
        </div>
    </div>
</body>
</html>
